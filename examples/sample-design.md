...
## Proposed Approach
### Overview
在登录页增加 magic link 入口；请求接口在校验邮箱后生成随机一次性令牌并写入短期存储，邮件服务发送带签名参数的登录链接；验证接口校验令牌、状态、有效期和风控信息，成功后复用现有 session 创建逻辑完成登录，并将令牌标记为已使用。
...
## Data Changes
### Schema / Model
- Add: magic_link token record in Redis with user_id, expires_at, used=false, redirect_target
- Modify: none
- Remove: none
...
## Need Human Decision
- 是否要求 magic link 仅对已验证邮箱且近 90 天活跃用户开放首发？
