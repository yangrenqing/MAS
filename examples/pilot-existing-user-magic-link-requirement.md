...
- Type: feature
- Priority: P1
- Source: starter pilot baseline
- Requested By: Growth Team
- Intake Owner: Intake Agent

## Summary
为现有注册用户增加邮箱 magic link 登录方式，减少忘记密码导致的登录失败和支持工单，同时不替代当前密码登录，并确保首发路径支持 feature flag、canary 和 rollback。
...
## Initial Scope
### In Scope
- Web 端为现有用户提供 magic link 登录入口。
- 发送一次性登录链接到用户邮箱。
- 用户点击链接后完成登录并跳转回原目标页面。
- 链接具备有效期、单次使用限制和基础频率限制。
- 首发能力支持 feature flag 与小流量 canary。
...
- 邮件送达与延迟会影响登录成功率。
- 若链接可重复使用、redirect 校验不严或暴露账户存在性，会带来安全风险。

## Missing Information
- magic link 有效期需要由产品和安全共同确认。
- 是否只对已验证邮箱用户开放首发仍需确认。
- 首次 canary 起始流量比例和观察窗口仍需确认。

## Need Human Decision
- 首发是否仅对已验证邮箱用户开放？
- 首次发布是否走 feature flag + canary，并从 5% 还是 10% 流量起步？
